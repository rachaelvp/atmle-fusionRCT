...
for(i in 61:B){
  ...
  print(paste0("tmle_res_slQ_glmG for sim ", i, " time: ", as.numeric((proc.time()-start)["elapsed"])/60))
  ...
  start <- proc.time()
  save(res_list, file = paste0("~/atmle-fusionRCT/output/res",i,".Rdata"))
...
for (i in 1:B) {
  print(i)
  load(paste0("~/atmle-fusionRCT/output/res",i,".Rdata"))
  result_list[[i]] <- res_list
}
...
write.csv(res, file = "~/atmle-fusionRCT/results_simple.csv", row.names = F)
