...
save(seeds, file = "~/atmle-fusionRCT/simulations/output/seeds.Rdata")
...
for(i in 1:B){
  ...
  # tmle_res_slQ <- tmle(
  #   Y = d_S$Y, A = d_S$A, W = d_S[, c("W1", "W2")], 
  #   g1W = rep(0.5, nrow(d_S)),
  #   family = "gaussian"
  # )
  # res_tmle_slQ <- data.frame(
  #   psi=tmle_res_slQ$estimates$ATE$psi, se=sqrt(tmle_res_slQ$estimates$ATE$var.psi)
  # )
  # timer <- as.numeric((proc.time()-start)["elapsed"])/60
  # print(paste0("tmle_res_slQ for sim ", i, " time: ", timer))
  ...
  tmle_res_glmQ_glmG <- tmle(
    Y = d_S$Y, A = d_S$A, W = d_S[, c("W1", "W2")], 
    g1W = c("SL.glm"), Q = tmle_res_glmQ$Qinit$Q,
    family = "gaussian"
  )
  ...
  timer <- as.numeric((proc.time()-start)["elapsed"])/60
  print(paste0("tmle_res_slQ_glmG for sim ", i, " time: ", timer))
  ...
  #   family = "gaussian"
  # )
  # res_tmle_slQ_glmG <- data.frame(
  #   psi=tmle_res_slQ_glmG$estimates$ATE$psi, se=sqrt(tmle_res_slQ_glmG$estimates$ATE$var.psi)
  # )
  # timer <- as.numeric((proc.time()-start)["elapsed"])/60
  # print(paste0("tmle_res_slQ for sim ", i, " time: ", timer))
  ...
  start <- proc.time()
  save(res_list, file = paste0("~/atmle-fusionRCT/simulations/output/res_simple",i,".Rdata"))
...
for (i in 1:B) {
  print(i)
  load(paste0("~/atmle-fusionRCT/simulations/output/res_simple",i,".Rdata"))
  result_list[[i]] <- res_list
}
...
write.csv(res, file = "~/atmle-fusionRCT/simulations/results_simple.csv", row.names = F)
