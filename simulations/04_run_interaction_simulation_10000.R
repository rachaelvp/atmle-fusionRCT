...
# retain the B RNG seeds (saved in previous simulation study)
...
result_list <- foreach(i = 1:B, .verbose = T) %dopar% {
  ...
  res_tmle_glmQ_glmG <- data.frame(
    psi=tmle_res_glmQ_glmG$estimates$ATE$psi, se=sqrt(tmle_res_glmQ_glmG$estimates$ATE$var.psi)
  )
  timer <- as.numeric((proc.time()-start)["elapsed"])/60
  print(paste0("tmle_res_glmQ_glmG for sim ", i, " time: ", timer))
  ...
  res_list <- list(
    res_atmle = res_atmle, 
    res_atmle_glmnet = res_atmle_glmnet, 
  ...
  start <- proc.time()
  save(res_list, file = paste0("~/atmle-fusionRCT/simulations/output/ASinter10000_res",i,".Rdata"))
  timer <- as.numeric((proc.time()-start)["elapsed"])/60
  print(paste0("saving sim ", i, " time: ", timer))
...
result_list <- list()
for (i in 1:B) {
  load(paste0("~/atmle-fusionRCT/simulations/output/ASinter10000_res",i,".Rdata"))
...
atmle_stats <- summary_stats(
  estimator_name = "ATMLE", psi = atmle_res$psi, 
  se = atmle_res$se, psi_0 = psi0S_interaction
)
atmle_glmnet_stats <- summary_stats(
  estimator_name = "ATMLE_glmnet", psi = atmle_res_glmnet$psi, 
  se = atmle_res_glmnet$se, psi_0 = psi0S_interaction
)
atmle_hal_stats <- summary_stats(
  estimator_name = "ATMLE_HAL", psi = atmle_res_hal$psi, 
  se = atmle_res_hal$se, psi_0 = psi0S_interaction
)
tmle_glmQ_stats <- summary_stats(
  estimator_name = "TMLE_glmQ", psi = tmle_glmQ_res$psi, 
  se = tmle_glmQ_res$se, psi_0 = psi0S_interaction
)

tmle_glmQ_glmG_stats <- summary_stats(
  estimator_name = "TMLE_glmQ_glmG", psi = tmle_glmQ_glmG_res$psi, 
  se = tmle_glmQ_glmG_res$se, psi_0 = psi0S_interaction
)
unadj_stats <- summary_stats(
  estimator_name = "Unadjusted", psi = unadj_res$psi, 
...
write.csv(res, file = "~/atmle-fusionRCT/simulations/results_ASinteraction10000.csv", row.names = F)
